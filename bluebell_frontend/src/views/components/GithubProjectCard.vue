...
export default {
	...
	methods: {
		...
		getRequestUrl(language) {
			const urls = {
				all: `https://api.github.com/search/repositories?q=stars:>1&sort=stars&order=${this.sortType}&per_page=${this.pageSize}&page=${this.pageNumber}`,
				goLang: `https://api.github.com/search/repositories?q=stars:>1+language:go&sort=stars&order=${this.sortType}&per_page=${this.pageSize}&page=${this.pageNumber}`
			};
			let url = urls['all'];
		...
		async getProject() {
			...
			try {
				...
				this.$message.error('获取 GitHub 项目失败，请稍后重试');
			}
		},
		async getMoreProject() {
			...
			try {
				...
				const data = await response.json();
				if (data.items) {
					this.projectList = [...new Set(this.projectList.concat(data.items))];
				} else {
					throw new Error('获取更多数据失败');
				}
			} catch (error) {
				console.error('获取更多 GitHub 项目失败:', error);
				this.$message.error('获取更多项目失败，请稍后重试');
			} finally {
				this.loading = '';
			}
		},
